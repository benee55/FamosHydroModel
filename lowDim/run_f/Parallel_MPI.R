...
#
#######
# For study: ens = 1007; niter = 6 ; Proposal Matrix scaling 0.01
#######
# Arguments
...
if(cycle==1){
  source("run_f/Initialize.R") 
}else{
  load(paste("output_f/temperVal_",cycle-1,".RData",sep=""))
  if(temperVal$cumulative>0.98){stop("Stopping Criterion Met")}
  MCMCtemperVal<-temperVal$cumulative
  load(paste("output_f/mhParameters_",cycle-1,".RData",sep=""))
...
foreach::foreach(jobNum=1:ens) %dopar% {
  ...
  source("run_f/mcmc_source_Tr.R")
  ################################################################################
  load(paste("output_f/rsParameters_",cycle,".RData",sep=""))
  # MCMC
  niter.mcmc = niter
  par.init<-parMat[jobNum,]
  load(paste("output_f/temperVal_",cycle,".RData",sep=""))
  MCMCtemperVal<-temperVal$cumulative
  ...
  save(amcmc.out,MCMCtemperVal,temperVal,
       file=paste("/glade/scratch/sanjib/run_f/output/MCMC_",cycle,"_1_",jobNum,".RData",sep=""))
  ################################################################################
  rm(list=setdiff(ls(), c("ens","cycle","niter","inputDir","outputDir")))
}
