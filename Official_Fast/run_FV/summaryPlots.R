rm(list=ls()) 
setwd("~/Dropbox/FamosHydroModel/Official_Fast/output_FV/")
...
plot(dens[[1]], 
     xlim=range(dens[[1]]$x , dens[[2]]$x , dens[[3]]$x , dens[[4]]$x) ,  
...
for(k in 1:ncol(extremeOutput[[1]])){
  dens<-list()
  for(h in 1:4){dens[[h]]<-density(extremeOutput[[h]][,k])}
  plot(dens[[1]], 
       xlim=range(dens[[1]]$x , dens[[2]]$x , dens[[3]]$x , dens[[4]]$x ) ,  
       ylim=range(dens[[1]]$y , dens[[2]]$y , dens[[3]]$y , dens[[4]]$y ), 
       main=extremeDate[k])
...
par(mfrow=c(5,5), mar=c(2,2,2,2))
for(k in 1:ncol(extremeOutput[[1]])){
  dens<-density(extremeOutput[[4]][,k])
  plot(dens, 
