...
if(cycle==1){
  source("run_rep_var/Initialize.R") 
}else{
  load(paste("output_rep_var/temperVal_",cycle-1,".RData",sep=""))
  if(temperVal$cumulative>0.98){stop("Stopping Criterion Met")}
  MCMCtemperVal<-temperVal$cumulative
  load(paste("output_rep_var/mhParameters_",cycle-1,".RData",sep=""))
...
foreach::foreach(jobNum=1:ens) %dopar% {
  ...
  source("run_rep_var/mcmc_source_Tr.R")
  ################################################################################
  load(paste("output_rep_var/rsParameters_",cycle,".RData",sep=""))
  # MCMC
  niter.mcmc = niter
  par.init<-parMat[jobNum,]
  load(paste("output_rep_var/temperVal_",cycle,".RData",sep=""))
  MCMCtemperVal<-temperVal$cumulative
